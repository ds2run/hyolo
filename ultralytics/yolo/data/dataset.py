...
from ultralytics.nn.modules import extract_args_from_cli
...
class YOLODataset(BaseDataset):
    ...
    def __init__(self,
        ...
        self.labels_dir_name = DEFAULT_CFG.labels_dir_name
        ...
        super().__init__(img_path, imgsz, cache, augment, hyp, prefix, rect,
                         batch_size, stride, pad, single_cls, classes)
    ...
    def cache_labels(self, path=Path('./labels.cache')):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg, hier in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
        ...
        if nf == 0:
            LOGGER.warning(
                f'{self.prefix}WARNING ?? No labels found in {path}. {HELP_URL}'
            )
        ...
        if is_dir_writeable(path.parent):
            ...
            LOGGER.warning(
                f'{self.prefix}WARNING ?? Cache directory {path.parent} is not writeable, cache not saved.'
            )
        return x

    def get_labels(self):
        self.label_files = img2label_paths(self.im_files)  # original
        label_dir_name = self.labels_dir_name

        # Construct base_path and label_path
        base_path = os.path.dirname(os.path.dirname(
            self.img_path))  # Strip 'images/train'

        all_overrides = extract_args_from_cli()

        # Extract label_dir_name if available in CLI
        label_dir_name_cli = next(
            (item.split('=')[1]
             for item in all_overrides if item.startswith('labels_dir_name=')),
            None)

        if len(sys.argv) > 1 and "detect" in sys.argv:
            if label_dir_name_cli is not None:
                label_dir_name = str(label_dir_name_cli)

        label_path = os.path.join(
            base_path, label_dir_name)  # Append custom label directory

        # Replace the default "labels" with label_dir_nanme
        self.label_files = [
            str(Path(f).parent).replace('/labels', f'/{label_dir_name}') +
            '/' + Path(f).name for f in self.label_files
        ]
        ...
        try:
            import gc
            gc.disable(
            )  # reduce pickle load time https://github.com/ultralytics/ultralytics/pull/1585
            cache, exists = np.load(
                str(cache_path), allow_pickle=True).item(), True  # load dict
            gc.enable()
            assert cache[
                'version'] == self.cache_version  # matches current version
            assert cache['hash'] == get_hash(self.label_files +
                                             self.im_files)  # identical hash
        ...
        lengths = ((len(lb['cls']), len(lb['bboxes']), len(lb['segments']))
                   for lb in labels)
...
# TODO: use hyp config to set all these augmentations
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            ...
            if k in ['masks', 'keypoints', 'bboxes', 'cls', 'hierarchy_names']:
                value = torch.cat(value, 0)
            new_batch[k] = value
        new_batch['batch_idx'] = list(new_batch['batch_idx'])
        for i in range(len(new_batch['batch_idx'])):
            new_batch['batch_idx'][
                i] += i  # add target image index for build_targets()
