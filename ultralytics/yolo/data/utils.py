    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                hier = np.array([x[:len(x) - 4] for x in lb], dtype=np.float32)
                lb = np.array([x[-5:] for x in lb], dtype=np.float32)
        ...
        lb = lb[:, :5]
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg, hier
