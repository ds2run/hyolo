...
class BaseValidator:
    ...
    def __init__(self,
        ...
        (self.save_dir /
         'labels' if self.args.save_txt else self.save_dir).mkdir(
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            self.model = model
            self.loss = torch.zeros_like(trainer.loss_items,
            ...
            if isinstance(self.args.data,
                          str) and self.args.data.endswith('.yaml'):
            ...
            model.eval()
            model.warmup(imgsz=(1 if pt else self.args.batch, 3, imgsz,
                                imgsz))  # warmup
        ...
        for batch_i, batch in enumerate(bar):
            ...
            with dt[2]:
                if self.training:
                    ...
                    if self.dependency_loss:
                        self.bce_no_dependency = trainer.criterion(
                            preds, batch)[2]
                        self.punishment_normalized = trainer.criterion(
                            preds, batch)[3]
            ...
            if self.args.plots and batch_i < 30:
                self.plot_val_samples(batch, batch_i)
                self.plot_predictions(batch, preds, batch_i)

            self.run_callbacks('on_val_batch_end')
        stats = self.get_stats()
        if self.args.calc_set_metric and not self.training:
            self.set_metric = self.calculate_set_metrics()
            df, df_nones = self.count_numbers()
            print("number of predictions to number of objects: ")
            print(df)
            print("number of nones to number of objects: ")
            print(df_nones)
            print(f"self.set_metric: {self.set_metric}")
        ...
        if self.training:
            ...
            for l in range(len(stats)):
                results[f"results_level{l}"] = {
                    **stats[f'results_dict_level{l}'],
                    **trainer.label_loss_items(self.loss.cpu() / len(self.dataloader),
                                               prefix='val')
                }
                results[f"results_level{l}"] = {
                    k: round(float(v), 5)
                    for k, v in results[f"results_level{l}"].items()
                }
            return results
